...
export const exportPlanningToPDF = ({
  ...
  const percentages = !showAssignmentsOnly ? calculatePercentages(desiderata, startDate, endDate) : null;
  ...
  const colors = {
    primary: {
      bg: [255, 205, 210], // Rouge clair
      text: [183, 28, 28],  // Rouge foncé
      border: [0, 0, 0]     // Noir pour la bordure
    },
    secondary: {
      bg: [207, 226, 255], // Bleu clair
      text: [30, 64, 175],  // Bleu foncé
      border: [0, 0, 0]     // Noir pour la bordure
    },
    comment: {
      bg: [255, 251, 235], // Jaune très clair (bg-yellow-50)
      text: [0, 0, 0],     // Noir
      border: [0, 0, 0]    // Noir pour la bordure
    },
    grayed: {
    ...
    autoTable(doc, {
      ...
      didParseCell: (data) => {
        if (data.section === 'body') {
          ...
          data.cell.styles.halign = data.column === 0 ? 'left' : 'center';
...
export const exportAllPlanningsToPDFZip = async (
  ...
  for (const user of users) {
    ...
    const percentages = calculatePercentages(exportOptions.selections, startDate, endDate);
