...
const EditUserModal: React.FC<EditUserModalProps> = ({ user, isOpen, onClose, onSave }) => {
  const [roles, setRoles] = useState<{ isAdmin: boolean; isUser: boolean; isPartTime: boolean; isCAT: boolean } | null>(null);
            ...
            <div className="space-y-2">
              ...
              <label className="flex items-center">
                <input
                  ...
                  onChange={(e) => setRoles(prev => ({ ...prev, isManager: e.target.checked }))}
                  className="rounded border-gray-300 text-indigo-600 focus:ring-indigo-500 h-4 w-4"
                  disabled={isSubmitting}
                />
              ...
              <label className="flex items-center">
                <input
                  ...
                  onChange={(e) => setRoles(prev => ({ ...prev, isPartTime: e.target.checked }))}
                  className="rounded border-gray-300 text-indigo-600 focus:ring-indigo-500 h-4 w-4"
                  disabled={isSubmitting}
                />
              ...
              <label className="flex items-center">
                <input
                  type="checkbox"
                  checked={roles.isCAT}
                  onChange={(e) => setRoles(prev => ({ ...prev, isCAT: e.target.checked }))}
                  className="rounded border-gray-300 text-indigo-600 focus:ring-indigo-500 h-4 w-4"
                  disabled={isSubmitting}
                />
