...
import ShiftStatusBadge, { ShiftStatus } from '../ShiftStatusBadge';
...
const GroupedShiftExchangeList: React.FC<GroupedShiftExchangeListProps> = ({
  ...
  const groupedExchanges = React.useMemo(() => {
    ...
    exchanges.forEach(exchange => {
      ...
      if (bagPhaseConfig.phase === 'completed' && exchange.userId !== user?.id) {
        return;
      }
    ...
    return grouped;
  }, [exchanges, bagPhaseConfig.phase, user?.id]);
