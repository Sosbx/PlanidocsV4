...
const ExchangeProposalsModal: React.FC<ExchangeProposalsModalProps> = ({
  ...
  useEffect(() => {
    if (exchange && exchange.operationType) {
      setSelectedOperationTypes([exchange.operationType]);
    } else if (exchange) {
      // Si l'échange n'a pas de type d'opération spécifique, essayer de le déterminer
      const operationTypes: OperationType[] = [];
      
      // Vérifier le type de collection
      if (exchange.exchangeType === 'direct') {
        ...
        } else if (exchange.operationType === 'replacement') {
          operationTypes.push('replacement');
        }
      }
      
      setSelectedOperationTypes(operationTypes);
    }
  }, [exchange]);
  ...
  // Utiliser la fonction formatPeriod pour standardiser l'affichage des périodes
                ...
                <div className="space-y-4 max-h-60 overflow-y-auto">
                  ...
                  {localProposals.some(p => p.proposalType === 'exchange' || p.proposalType === 'both') && (
                    <div className="rounded-md border border-indigo-200 overflow-hidden">
                      ...
                      <div className="divide-y divide-gray-100">
                        {/* Les propositions d'échange sont filtrées ci-dessous */}
                        ...
                        {localProposals
                          ...
                          .map(proposal => {
                            ...
                            // Cela permettra d'accepter/rejeter chaque garde proposée indépendamment
                            return proposal.proposedShifts.map((shift, index) => {
                              ...
                              const colorClass = periodColors[shift.period as 'M' | 'AM' | 'S'] || 'bg-gray-50 text-gray-700 border-gray-200';
                              
                              // Créer un ID unique pour chaque proposition de garde spécifique
                              // Cela permettra d'identifier et de traiter chaque garde individuellement
