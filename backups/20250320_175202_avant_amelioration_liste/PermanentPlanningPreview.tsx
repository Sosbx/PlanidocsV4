  ...
  return (
    <div className={`bg-white rounded-lg shadow-lg overflow-hidden transform transition-all duration-300 w-full flex flex-col ${className} ${isSyncActive ? 'sync-active scale-[1.02]' : ''} sticky-container`}>
      ...
      <div className="px-3 py-2 border-b border-gray-200 flex justify-between items-center bg-indigo-50">
        <h3 className="text-sm font-semibold text-indigo-700 planning-title">Planning</h3>
        ...
        <table ref={tableRef} className="w-full text-[10px] sm:text-xs planning-transition">
          <thead>
            <tr>
              ...
              <th className="py-1.5 px-1 text-center font-medium text-amber-600 bg-amber-50/50 w-[23%]">Matin</th>
              <th className="py-1.5 px-1 text-center font-medium text-sky-600 bg-sky-50/50 w-[23%]">AM</th>
              <th className="py-1.5 px-1 text-center font-medium text-violet-600 bg-violet-50/50 rounded-tr-md w-[23%]">Soir</th>
            </tr>
          </thead>
          <tbody>
            {dates.map(date => {
                ...
                >
                  <td className="py-1 sm:py-2 px-1 sm:px-2">
                    <div className={`${isSelected ? 'bg-indigo-50 rounded px-1.5 py-0.5 ring-1 ring-indigo-200' : ''}`}>
                      <span className={`text-[11px] sm:text-xs font-semibold ${isSelected ? 'text-indigo-700' : isToday ? 'text-yellow-700' : isWeekend ? 'text-red-600' : ''}`}>
      ...
      {/* Barre de commandes simplifiée - réduite au minimum nécessaire */}
      <div 
        ref={dateLabelRef}
        className="px-3 py-2 bg-white border-t border-gray-200 flex items-center justify-center text-xs"
      >
