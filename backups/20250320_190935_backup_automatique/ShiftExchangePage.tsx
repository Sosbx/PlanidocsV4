...
const ShiftExchangePage: React.FC = () => {
  ...
  // État de navigation du calendrier - sera initialisé à la première date disponible
  ...
  const [calendarViewMode, setCalendarViewMode] = useState<'month' | 'week'>('month'); // Mode mois par défaut
  ...
  // Fonctions de navigation du calendrier - toujours par mois
  const goToPrevious = useCallback(() => {
    // Toujours naviguer par mois, quelle que soit la vue
  ...
  // Gestionnaires d'événements pour le swipe - version améliorée
  const handleTouchStart = useCallback((e: TouchEvent) => {
    if (viewMode !== 'calendar') return;
    if (!e.touches?.length || e.touches.length !== 1) return;
    
    // Capturer les positions de départ
    ...
    // Log pour débogage
    console.log("Touch start captured:", swipeInfo.current);
  }, [viewMode]);
  ...
  const handleTouchEnd = useCallback((e: TouchEvent) => {
    ...
    // S'assurer que les coordonnées de départ existent
    if (touchStartX === 0 && touchStartY === 0) return;
    
    const elapsedTime = Date.now() - touchStartTime;
    if (elapsedTime > 500) return; // Augmenter légèrement le délai
    ...
    // Log pour débogage
    console.log("Touch swipe detected:", { distX, distY, elapsedTime });
    
    // Assouplir les critères pour déclencher le swipe
    if (Math.abs(distX) >= 50 && Math.abs(distY) <= 150) {
      ...
      // Réinitialiser les valeurs après un swipe
      swipeInfo.current = { touchStartX: 0, touchStartY: 0, touchStartTime: 0 };
    }
  ...
  useEffect(() => {
    const checkScreenSize = () => {
      ...
      // Toujours en mode mois par défaut, quelle que soit la taille de l'écran
  ...
  // useEffect pour les événements tactiles - version améliorée
  useEffect(() => {
    // Récupérer le conteneur une fois que le composant est monté
    const container = calendarContainerRef.current;
    if (!container) {
      console.warn("Calendar container ref not available");
      return;
    }
    
    console.log("Adding touch event listeners to calendar container");
    ...
    // Ajouter touchcancel pour gérer les cas où le toucher est interrompu
    ...
    try {
      ...
      const unsubscribeMain = onSnapshot(
        ...
        (snapshot) => {
          ...
          // Initialiser le mois du calendrier à la première date disponible
          if (exchangeData.length > 0) {
            const firstExchangeDate = new Date(exchangeData[0].date);
            setCurrentMonth(firstExchangeDate);
          }
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-6">
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <div className="flex border border-gray-200 rounded-full overflow-hidden">
              <button 
                ...
                disabled={isInteractionDisabled}
                className={`text-[9px] px-1.5 py-0.5 ${selectedPeriod === 'all' ? 'bg-indigo-100 text-indigo-700' : 'bg-white text-gray-600'} ${isInteractionDisabled ? 'opacity-50 cursor-not-allowed' : ''}`}
              >
                Tous
              </button>
              <button 
                ...
                disabled={isInteractionDisabled}
                className={`text-[9px] px-1.5 py-0.5 border-l border-r border-gray-200 ${selectedPeriod === 'M' ? 'bg-amber-100 text-amber-700' : 'bg-white text-gray-600'} ${isInteractionDisabled ? 'opacity-50 cursor-not-allowed' : ''}`}
              >
                M
              </button>
              <button 
                ...
                disabled={isInteractionDisabled}
                className={`text-[9px] px-1.5 py-0.5 border-r border-gray-200 ${selectedPeriod === 'AM' ? 'bg-sky-100 text-sky-700' : 'bg-white text-gray-600'} ${isInteractionDisabled ? 'opacity-50 cursor-not-allowed' : ''}`}
              >
                AM
              </button>
              <button 
                onClick={() => setSelectedPeriod('S')} 
                disabled={isInteractionDisabled}
                className={`text-[9px] px-1.5 py-0.5 ${selectedPeriod === 'S' ? 'bg-violet-100 text-violet-700' : 'bg-white text-gray-600'} ${isInteractionDisabled ? 'opacity-50 cursor-not-allowed' : ''}`}
              >
                S
              </button>
            </div>
      ...
      {!user || loading ? (
        ...
        <div className="flex flex-row gap-1 sm:gap-3 md:gap-4">
        ...
        // Vue Agenda améliorée sans planning flottant - avec support tactile amélioré
        <div 
          ...
          <div className="p-4">
                            ...
                            {['M', 'AM', 'S'].map(period => {
                              ...
                              return (
                                <div key={`period-${period}`} className={`${isMobile ? 'mb-0.5 relative pl-1.5' : 'mb-1'}`}>
                                  {/* Suppression des barres latérales de période */}
                                      ...
                                      return (
                                        <div 
                                          ...
                                          className={`
                                            ...
                                            rounded-sm bg-gray-100 text-black border border-gray-300 border-dotted shadow-none opacity-50 grayscale
                                          `}
                                          title="Ma garde"
                                        >
                                          ...
                                          <button
                                            ...
                                            className={`
                                              ...
                                              ${exchange.userId === user?.id 
                                                ? 'opacity-40 scale-90 border-dotted z-0 grayscale bg-gray-100/80 text-gray-600 border-gray-300' 
                                                : 'scale-110 shadow-sm z-10 border-solid'
                                              }
                                              ${(isInterested) ? 'shadow-md ring-2 ring-opacity-60 z-20 ' + (hasConflict ? 'ring-red-500' : 'ring-green-500') : ''}
                                              ${isInterested && isMobile ? 'scale-120' : ''}
                                            `}
            ...
            <div className="mt-2 border-t border-gray-200 pt-1 flex flex-wrap items-center justify-between text-[9px] text-gray-500">
              <div className="flex gap-2">
                <span className="flex items-center"><span className="inline-block h-2 w-2 bg-amber-300 rounded-full mr-1"></span>M</span>
                <span className="flex items-center"><span className="inline-block h-2 w-2 bg-sky-300 rounded-full mr-1"></span>AM</span>
                <span className="flex items-center"><span className="inline-block h-2 w-2 bg-violet-300 rounded-full mr-1"></span>S</span>
              </div>
              
              <div className="flex gap-2 flex-wrap">
                <span className="flex items-center"><span className="inline-block h-2 w-2 opacity-50 bg-gray-300 border border-dotted border-gray-400 rounded-full mr-1"></span>Mes gardes</span>
