...
// Version: 1.0.2 - Désactivé pour desktop, mobile uniquement

// Vérifier si c'est un mobile
const userAgent = self.navigator?.userAgent || '';
const isMobile = /iPhone|iPad|iPod|Android/i.test(userAgent);

// Si desktop, ne pas exécuter le service worker
if (!isMobile) {
  console.log('💻 Desktop détecté - Service Worker notifications désactivé');
  // Répondre aux événements mais sans traiter les notifications
  ...
  // Arrêter ici pour desktop
} else {
  // Mobile uniquement - charger Firebase et gérer les notifications
  console.log('📱 Mobile détecté - Chargement du Service Worker notifications');
  ...
  console.log(`[SW v${SW_VERSION}] Firebase Messaging Service Worker chargé (mobile uniquement)`);
}